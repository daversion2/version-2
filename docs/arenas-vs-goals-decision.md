...
| **3** | Proof layer **in parallel**: Override Score (re-bucket completionLogs by `item.arena_id`), Discipline Map (re-key the dead `getGoalBreakdown`), baseline-test flow (reuse measurements pattern, **no new index**), **+ per-completion resistance capture.** Run alongside Follow-Through. **Then watch telemetry.** **→ Full plan in [phase-3-proof-layer.md](./phase-3-proof-layer.md).** | Low (non-destructive, parallel) | OTA |
