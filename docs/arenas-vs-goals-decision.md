...
| **3** 🔶 | **3.1–3.3 DONE (2026-06-21); 3.4 (telemetry/observe) remains.** Override Score + Discipline Map on Progress, the resistance-prompt reframe, and Arena Baselines + Discomfort Shift (Stopwatch, baseline store, Baseline Test + Arena Detail screens). All non-destructive, alongside Follow-Through. **→ Full plan + status in [phase-3-proof-layer.md](./phase-3-proof-layer.md).** | Low (non-destructive, parallel) | OTA |
