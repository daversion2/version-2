...
| **1** 🔶 | **In progress (2026-06-21).** ✅ Optional `arena_id` on all 4 entities + `createHabit` signature; ✅ `ArenaPicker` (single-select) built; ✅ required in the manual challenge-creation flow; ✅ auto-derive from library on challenge-use + habit-add; ✅ arena chips on active challenge/habit detail (visibility). **Deferred:** ArenaPicker in the other human creation/edit flows (custom habit, EditChallenge, programs, worksheets, Tools), planner auto-derive (`convertPlannedChallengesToChallenges`), and program/worksheet create paths. | Low (additive, dormant if abandoned) | OTA |
| **2** | Migrate CBT/why off `goals[0]` → user-level profile; redirect `saveGoalCBTData` there. | Low, isolates the fragile coupling | OTA |
| **3** | Proof layer **in parallel**: Override Score (re-bucket completionLogs by `item.arena_id`), Discipline Map (re-key the dead `getGoalBreakdown`), baseline-test flow (reuse measurements pattern, **no new index**), **+ per-completion resistance capture.** Run alongside Follow-Through. **Then watch telemetry.** | Low (non-destructive, parallel) | OTA |
