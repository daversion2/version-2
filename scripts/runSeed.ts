/**
 * Simple script to seed the challenge library
 * Run with: npx ts-node scripts/runSeed.ts
 */

import { initializeApp } from 'firebase/app';
import { getFirestore, collection, addDoc, getDocs } from 'firebase/firestore';

// Firebase config - same as in src/services/firebase.ts
const firebaseConfig = {
  apiKey: process.env.EXPO_PUBLIC_FIREBASE_API_KEY || 'AIzaSyBKMOE7bDinoVHWv_S2t0VdYTB8ACVO8HU',
  authDomain: process.env.EXPO_PUBLIC_FIREBASE_AUTH_DOMAIN || 'neuro-nudge-62b4d.firebaseapp.com',
  projectId: process.env.EXPO_PUBLIC_FIREBASE_PROJECT_ID || 'neuro-nudge-62b4d',
  storageBucket: process.env.EXPO_PUBLIC_FIREBASE_STORAGE_BUCKET || 'neuro-nudge-62b4d.firebasestorage.app',
  messagingSenderId: process.env.EXPO_PUBLIC_FIREBASE_MESSAGING_SENDER_ID || '707539028220',
  appId: process.env.EXPO_PUBLIC_FIREBASE_APP_ID || '1:707539028220:web:32ef320f3b360dc4d88aa9',
};

const app = initializeApp(firebaseConfig);
const db = getFirestore(app);

interface ChallengeVariation {
  label: string;
  description: string;
}
...
const CHALLENGES: SeedChallenge[] = [
  {
    ...
    common_resistance: ["It's too cold", "I'll do it tomorrow", "This is pointless"],
    real_world_examples: ['At the sink after brushing teeth', 'In the shower before warming up', 'During lunch break to reset focus'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ['I need to check something important', "It's boring without my phone", "I'll just look at it for a second"],
    real_world_examples: ['Breakfast before work', 'Lunch at your desk (phone in drawer)', 'Dinner with family'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["I'm tired", "It's just one elevator ride", 'No one will know if I take the easy way'],
    real_world_examples: ['Office building', 'Parking garage', 'Shopping mall', 'Apartment complex'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ['They look busy', "I don't have anything interesting to say", "They'll think I'm weird"],
    real_world_examples: ['Comment on something to the person next to you in line', 'Ask someone at the gym about their workout', "Compliment someone's dog at the park"],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["I don't have time for this", 'This is boring', 'I should be doing something productive'],
    real_world_examples: ['First thing in the morning before checking phone', 'At your desk before starting work', 'In your car before going inside'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["I'm going to sleep in it again anyway", "It doesn't matter", "I'll do it later"],
    real_world_examples: ['Immediately after getting out of bed', 'Before using the bathroom', 'Before checking your phone']
  },
  {
    ...
    common_resistance: ["I'm just venting", 'But this is actually annoying', "I'm not complaining, I'm stating facts"],
    real_world_examples: ['Traffic → "I have time to listen to a podcast"', 'Bad weather → "I\'ll appreciate the nice days more"', 'Annoying coworker → Focus on what you can control'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ['Maybe just end with cold', "It's too early/late for this", "I'll do it when it's warmer outside"],
    real_world_examples: ['Morning shower to wake up', 'After workout to recover', 'Before a challenging day'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["That's embarrassing", "They'll say no anyway", "I don't want to be that person"],
    real_world_examples: ['Ask for 10% off at a retail store', 'Ask if there are any current promotions at a restaurant', 'Ask if a service fee can be waived']
  },
  {
    ...
    common_resistance: ["I'll do it when I know what to say", "It's been too long, it'll be awkward", 'I need more time to think'],
    real_world_examples: ["Text that friend you've lost touch with", "Respond to that email you've been avoiding", 'Reach out to apologize or clear the air']
  },
  {
    ...
    common_resistance: ['What if someone needs me', "I'll just check this one thing", '90 minutes is too long'],
    real_world_examples: ['Writing project', 'Learning a skill', 'Strategic thinking or planning', 'Creative work'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["But they'll be upset", "It's not that big a deal", "I don't want to seem selfish"],
    real_world_examples: ['Decline an optional meeting', "Say no to plans you don't want to attend", 'Turn down a request for your time']
  },
  {
    ...
    common_resistance: ["Life's too short to eat things I don't like", "There's no point", "I'll just eat more of what I do like"],
    real_world_examples: ['Vegetables you avoid', "A dish you've decided you don't like", 'Something with a texture that bothers you']
  },
  {
    ...
    common_resistance: ['I just need to distract myself', 'This is too much', 'Feeling bad is unproductive'],
    real_world_examples: ["Anxiety before a meeting - don't check your phone", "Frustration from an email - don't vent immediately", "Sadness - don't scroll to numb it"],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ['I need to ease into the day', 'Let me just check email first', "I'll have more energy later"],
    real_world_examples: ["The project you've been avoiding", 'The difficult conversation', 'The creative work that requires deep focus']
  },
  {
    ...
    common_resistance: ['That would be awkward', "They'll think I want something", "I don't want to seem like a suck-up"],
    real_world_examples: ['Tell a coworker specifically what they did well', "Compliment someone's approach to a problem", 'Thank someone for something specific they do'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ["I'll just check real quick", 'I might miss something important', "It's how I wake up"],
    real_world_examples: ['Leave phone charging in another room', 'Use app blockers until noon', 'Replace the habit with reading or walking'],
    variations: [
  ...
  },
  {
    ...
    common_resistance: ['Just five more minutes', "I didn't sleep well", 'The bed is so comfortable'],
    real_world_examples: ['Put alarm across the room', 'Have a reason to get up ready', 'Commit the night before'],
    variations: [
...
async function seed() {
  ...
  const existingDocs = await getDocs(libraryRef);
  console.log(`Current library has ${existingDocs.size} challenges`);

  console.log(`\nAdding ${CHALLENGES.length} challenges...`);

  let added = 0;
  for (const challenge of CHALLENGES) {
    try {
      await addDoc(libraryRef, challenge);
      console.log(`  ✓ ${challenge.name}`);
      added++;
    } catch (error) {
      console.error(`  ✗ ${challenge.name}:`, error);
    }
  }

  console.log(`\nDone! Added ${added} challenges.`);

  // Verify
  const finalDocs = await getDocs(libraryRef);
  console.log(`Library now has ${finalDocs.size} challenges total.`);
